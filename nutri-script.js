...
async function generateNutritionPlan() {
    ...
    try {
        ...
        // Display the plan
        renderNutritionResults(nutritionPlan.meals);
...
function showLoadingState() {
    ...
    resultsContainer.innerHTML = `
        <div class="flex flex-col justify-center items-center text-center py-12 w-full h-full">
            <div class="w-20 h-20 mb-6 bg-gradient-to-r from-blue-500 to-purple-600 rounded-full flex items-center justify-center animate-spin mx-auto">
...
function renderNutritionResults(meals) {
    const resultsContainer = document.getElementById('nutrition-results');
    resultsContainer.innerHTML = '';
    meals.forEach(meal => {
        const card = document.createElement('div');
        card.className = 'flip-card mb-2';
        card.innerHTML = `
            <div class="flip-card-inner">
                <div class="flip-card-front p-3 shadow-lg border border-gray-700 flex flex-col items-center max-w-full">
                    <img src="${meal.image}" alt="${meal.name}" class="rounded-xl w-full h-40 object-cover mb-4" />
                    <h3 class="text-xl font-bold mb-2 text-center">${meal.name}</h3>
                    <div class="flex flex-row items-center justify-center text-gray-400 text-xs mb-2 gap-2 w-full">
                        <span><strong>${meal.macros.calories}</strong> kcal</span>
                        <span>| Protein: <strong>${meal.macros.protein}g</strong></span>
                        <span>| Fat: <strong>${meal.macros.fat}g</strong></span>
                    </div>
                </div>
                <div class="flip-card-back p-3 shadow-lg border border-gray-700 flex flex-col justify-center items-center">
                    <h4 class="text-lg font-semibold mb-2">Benefits</h4>
                    <ul class="mb-2 text-sm text-green-300 list-disc list-inside text-left">
                        ${meal.benefits.map(b => `<li>${b}</li>`).join('')}
                    </ul>
                    <h4 class="text-lg font-semibold mb-2">Details</h4>
                    <p class="text-gray-200 text-xs mb-2">${meal.tips || ''}</p>
                    <p class="text-gray-400 text-xs">Nutrients: ${meal.nutrients ? meal.nutrients.join(', ') : ''}</p>
                </div>
            </div>
        `;
